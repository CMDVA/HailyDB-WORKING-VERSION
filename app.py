...
@app.route('/api/test-enhanced-context/<int:report_id>', methods=['POST'])
def test_enhanced_context_generation(report_id):
    """Simple Enhanced Context generation for testing and immediate backfill"""
    try:
        # Get the report
...
@app.route('/api/enhanced-context-backfill', methods=['POST'])
def enhanced_context_backfill():
    """100% Enhanced Context backfill for ALL SPC reports"""
    try:
        data = request.get_json() or {}
        batch_size = data.get('batch_size', 50)
        ...
        if total_reports == 0:
            return jsonify({
                "success": True,
                "message": "All reports already have Enhanced Context v2.0",
                "total_reports": 0,
                "processed": 0
            })
        ...
        db.session.rollback()
        logger.error(f"Enhanced Context backfill error: {e}")
        return jsonify({"success": False, "error": str(e)}), 500
