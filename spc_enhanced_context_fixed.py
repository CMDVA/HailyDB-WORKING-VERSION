...
from models import SPCReport, Alert, VerifiedMatch
...
class SPCEnhancedContextService:
    ...
    def _get_verified_alerts_for_report(self, report_id: int) -> List:
        ...
        verified_matches = self.db.query(VerifiedMatch).filter(
            VerifiedMatch.spc_report_id == report_id
        ).all()
        
        alerts = []
        for match in verified_matches:
            alert = self.db.query(Alert).filter(Alert.id == match.alert_id).first()
            if alert:
                alerts.append(alert)
